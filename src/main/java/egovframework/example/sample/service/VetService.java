...
public interface VetService {
	...
	List<?> searchVetByName(String hpt_name, Double latitude, Double longitude) throws Exception;

	// 지역명(시/도 + 시/군/구)을 이용해 동물 병원의 목록을 검색한다.
	List<?> searchVetByRegion(String province, String city, Double latitude, Double longitude) throws Exception;
	
	// 현재 나의 좌표를 기준으로 직선 거리가 가장 가까운 30개의 병원의 목록을 검색한다.
	List<?> searchNearest(Double latitude, Double longitude) throws Exception;
